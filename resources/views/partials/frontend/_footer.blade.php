<!-- Footer -->
<footer>
    <div class="parallax100 bg-1 pt-md-5 pt-3" style="background: black;">
        <div class="container">
            <div class="row">
                <div class="col-md-3 col-12">
                    <div class="text-md-left text-center  py-5">
                        <a href="{{ route('home') }}">
                            <img src="{{ asset('images/bnw/bnw-web-logo.png') }}" class="logo-footer" alt="LOGO"
                                style="height: auto;">
                        </a>
                    </div>
                </div>
                <div class="col-md-3"></div>
                <div class="col-md-3 text-left footer-padding">
                    <div class="my-3 py-3 text-white">
                        <p class="custom-font-avenir-medium"><span
                                class="font-weight-bold custom-font-avenir-medium">P</span>&nbsp;&nbsp;&nbsp;+62 822
                            9982 3888</p>
                        <p class="custom-font-avenir-medium"><span
                                class="font-weight-bold custom-font-avenir-medium">E</span>&nbsp;&nbsp;&nbsp;info@benandwyatt.com
                        </p>
                        <p class="mb-5 custom-font-avenir-medium"><span
                                class="font-weight-bold custom-font-avenir-medium">I</span>&nbsp;&nbsp;&nbsp;&nbsp;benandwyatt
                        </p>
                    </div>
                </div>
                <div class="col-md-3 text-md-left text-center">
                    <div class="my-3 py-3 text-white pl-md-5">
                        <p class="custom-font-avenir-medium">Brooklyn Alam Sutera<br />
                            Soho Tower A, 5th Floor<br />
                            A/05/C - A/05/D</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>